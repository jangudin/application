...
class Registrasiusermodel extends CI_Model{	
	...
	function getsisdmkid($id_faskes, $user_id=NULL)
	{
	
	   	$raw_user_id=" and NIK='".$user_id."' and id_faskes='".$id_faskes."' ";

		$sql = $this->db->query("SELECT id FROM data_sisdmk  WHERE 1=1 ".$raw_user_id." ORDER BY created_at DESC");
	...
	function getprofile($user_id=NULL)
	{
		...
		$sql = $this->db->query("SELECT registrasi_user.*,kategori.kategori_user,kategori.keterangan,trans_final.kode_faskes_baru
		FROM registrasi_user 
		LEFT JOIN kategori ON kategori.id=registrasi_user.id_kategori  
		LEFT JOIN trans_final ON trans_final.id_faskes=registrasi_user.id  
...
WHERE 1=1 ".$var_kategori." ".$var_where." ".$var_extra." ");
	...
	function get_labkes_jenis_pelayanan_dev($filters=NULL, $order=NULL)
	{

		$sql = $this->db->query(" SELECT * from master_labkes_jenis_pelayanan_copy1   WHERE  ".$filters." ORDER BY ".$order." ");
		return $sql->result_array();
	}
	
	function get_labkes_jenis_pelayanan_child($filters=NULL, $order=NULL)
	{

		$sql = $this->db->query(" SELECT * from master_labkes_jenis_pelayanan_child   WHERE  ".$filters." ORDER BY ".$order." ");
		return $sql->result_array();
	}
	...
	function getlistpendaftaranpuskesmas($id_kategori=NULL,$id_faskes=NULL,$id_kota=NULL,$id_prov=NULL)
	{
		...
		$sql = $this->db->query("SELECT registrasi_user.*,kategori.kategori_user,kategori.keterangan,(SELECT COUNT(*) AS jml FROM trans_final tf WHERE tf.id_faskes=registrasi_user.id AND tf.kode_faskes !='') AS jml,trans_final.kode_faskes, puskesmas_pusdatin.kode_baru  FROM registrasi_user 
		LEFT JOIN kategori ON kategori.id=registrasi_user.id_kategori 
		LEFT JOIN trans_final ON registrasi_user.id=trans_final.id_faskes 
		LEFT JOIN puskesmas_pusdatin ON registrasi_user.id_faskes = puskesmas_pusdatin.kode_sarana
	...
	function getdatastandarpelayananlabkes($user_id=NULL)
	{
	
	 
			$raw_user_id=" and trans_labkes_standar_pelayanan.id_faskes='".$user_id."' ";

		$sql = $this->db->query("SELECT trans_labkes_standar_pelayanan.* FROM trans_labkes_standar_pelayanan  WHERE 1=1 ".$raw_user_id." ");
		return $sql->result_array();
	}
	
	function getdatasdmlabkesmas($user_id=NULL)
	{
	
	 
			$raw_user_id=" and trans_labkes_sdm_kesmas.id_faskes='".$user_id."' ";

		$sql = $this->db->query("SELECT trans_labkes_sdm_kesmas.* FROM trans_labkes_sdm_kesmas  WHERE 1=1 ".$raw_user_id." ");
		return $sql->result_array();
	}
	...
	function getdatagambarpm($user_id=NULL)
	{
		$raw_user_id=" and a.id_faskes='".$user_id."' and a.server_penyimpanan='registrasifasyankes' ";

		$sql = $this->db->query("SELECT a.* FROM t_img_faskes a  WHERE 1=1 ".$raw_user_id." ");
		return $sql->result_array();
	}

	function deletedatagambarpm($id=NULL)
	{
		$raw_user_id=" and a.id='".$id."'  ";

		$sql = $this->db->query("DELETE FROM t_img_faskes a  WHERE 1=1 ".$raw_user_id." ");
		return $sql->result_array();
	}
	...
	kategori.keterangan,
	kategori.kategori_user,
	registrasi_user.id AS id_faskes,
	data_labkes.nama_lab AS nama_klinik,
	data_labkes.alamat_faskes,
	propinsi.nama_prop,
	kota.nama_kota 
FROM
	registrasi_user
	...
	LEFT JOIN propinsi ON registrasi_user.id_prov = propinsi.id_prop
	LEFT JOIN kota ON registrasi_user.id_prov = kota.id_prop 
	AND kota.id_kota = registrasi_user.id_kota
	LEFT JOIN kategori ON kategori.id = registrasi_user.id_kategori 
WHERE
	1 = 1 UNION ALL
SELECT
	...
	kategori.kategori_user,
	registrasi_user.id AS id_faskes,
	data_pm.nama_pm AS nama_klinik,
...
ORDER BY
	...
	function getlistpcare()
	{
     	$sql = $this->db->query("SELECT a.*,b.nama AS kedeputian
		FROM bridging_pcare a LEFT JOIN kedeputian b ON a.kedeputian_id = b.id  WHERE 1=1  ORDER BY a.id DESC ");
		return $sql->result_array();
	}
	...
	if($id_kategori=='1'){
		...
		$sql = $this->db->query("SELECT registrasi_user.*,kategori.kategori_user,kategori.keterangan ,trans_final.token_kode_faskes,trans_final.kode_faskes,data_klinik.nama_klinik,data_klinik.alamat_faskes,data_klinik.persalinan,data_klinik.status_klinik,data_klinik.alasan_status_klinik,trans_final.kode_faskes_baru
		FROM trans_final 
	...
	function findNoFaskesRS($id_link, $kategori, $kelas) {	
	...
	function getrekap_data($tgl1=null,$tgl2=NULL,$id_prov=NULL,$id_kota=NULL,$id_kategori=NULL,$jenis_klinik=NULL,$jenis_perawatan=NULL,$sorting=NULL,$type_sorting=NULL, $layananklinik=NULL)
	{
        ...
        if(!empty($layananklinik)){
           $layananklinik=" AND dk.pelayanan_klinik like '%".$layananklinik."%'";
        }else{
           $layananklinik="";
        }
		...
		,`dk`.`alamat_faskes_versi_akreditasi` AS `alamat_faskes_versi_akreditasi`
		,`dk`.`alamat_cleaning` AS `alamat_cleaning`
		...
		,`dk`.`latitude` AS `latitude`
        ,`dk`.`longitude` AS `longitude`
        ...
        where `trans_final`.`kode_faskes` is not null AND 1=1 ".$prov.$kota.$kategori.$jenis_klinik.$jenis_perawatan.$layananklinik." ORDER BY `trans_final`.`kode_faskes_baru` ASC ");
	...
	function getrekap_data_lab($tgl1=null,$tgl2=NULL,$id_prov=NULL,$id_kota=NULL,$id_kategori=NULL)
	{
		...
		$sql = $this->db->query("SELECT registrasi_user.*,kategori.kategori_user,kategori.keterangan,trans_final.token_kode_faskes,trans_final.kode_faskes,trans_final.kode_faskes_baru,data_labkes.nama_lab,data_labkes.alamat_faskes,propinsi.nama_prop,kota.nama_kota,data_labkes.no_telp,
		data_labkes.email AS email_lab,data_labkes.jenis_lab,data_labkes.bentuk_lab, data_labkes.status_labkes, data_labkes.pemilik
		FROM trans_final 
		INNER JOIN registrasi_user ON trans_final.id_faskes=registrasi_user.id 
		LEFT JOIN data_labkes ON data_labkes.id_faskes=registrasi_user.id 
		LEFT JOIN kategori ON registrasi_user.id_kategori=kategori.id 
	...
	function getrekap_data_pm($tgl1=null,$tgl2=NULL,$id_prov=NULL,$id_kota=NULL,$id_kategori=NULL)
	{
		...
		$sql = $this->db->query
		("SELECT registrasi_user.*,kategori_pm.kategori_user,kategori_pm.keterangan,trans_final.token_kode_faskes,trans_final.kode_faskes,data_pm.nama_pm,data_pm.alamat_faskes,data_pm.alamat_faskes,propinsi.nama_prop,kota.nama_kota,data_pm.status_pm, data_pm.no_telp,data_pm.no_hp,
		IF(data_pm.dokumen_komitmen is NOT NULL,'Ada', NULL) AS dokumen_komitmen,
	...
	function monitoring_pm($tgl1=null,$tgl2=NULL,$id_prov=NULL,$id_kota=NULL,$id_kategori=NULL)
	{
		...
		kategori.kategori_user,
		kategori.keterangan,
		trans_final.token_kode_faskes,
		trans_final.kode_faskes,
		data_pm.nama_pm,
		data_pm.alamat_faskes,
		propinsi.nama_prop,
		kota.nama_kota,
		data_pm.no_telp,
		data_pm.email AS email_pm,
		trans_final.status_validasi_kemkes,
		trans_final.status_validasi_prov,
		trans_final.status_validasi_kota,
		kategori_pm.kategori_user AS kategori_user_pm,
		data_sisdmk.id cek_sdm,
		data_rme.id cek_rme,
		data_rme.STATUS status_rme,
		data_pm.kerja_sama_bpjs_kesehatan,
		IF(tpmd_sudah_satusehat.id is NOT NULL,'Sudah', NULL) AS kirim_satu_sehat,
		IF(data_pm.dokumen_komitmen is NOT NULL,'Ada', NULL) AS dokumen_komitmen,
		CONCAT('https://registrasifasyankes.kemkes.go.id/assets/uploads/berkas_registrasi/',data_pm.dokumen_komitmen) as url_dokumen_komitmen,
		qrapi.id cek_qr 
	FROM
		...
		LEFT JOIN kategori ON registrasi_user.id_kategori = kategori.id
		LEFT JOIN propinsi ON propinsi.id_prop = registrasi_user.id_prov
		LEFT JOIN kota ON kota.id_prop = registrasi_user.id_prov_pm 
		AND kota.id_kota = registrasi_user.id_kota_pm
		LEFT JOIN kategori_pm ON registrasi_user.id_kategori_pm = kategori_pm.id
		LEFT JOIN ( SELECT id, id_faskes FROM data_sisdmk GROUP BY id_faskes, id ) data_sisdmk ON data_sisdmk.id_faskes = trans_final.id_faskes
		LEFT JOIN data_rme ON data_rme.id_faskes = trans_final.id_faskes
		LEFT JOIN qrapi ON qrapi.id_faskes = trans_final.id_faskes 
		LEFT JOIN tpmd_sudah_satusehat on tpmd_sudah_satusehat.kode_faskes = trans_final.kode_faskes
		WHERE 1=1 ".$prov.$kota." AND trans_final.kode_faskes !=''  ORDER BY trans_final.kode_faskes DESC
		");
		return $sql->result_array();
	}
	...
	function getdatapm($user_id=NULL)
	{
		...
		$sql = $this->db->query("SELECT data_pm.*,propinsi.kode_regional, c.nama_kota, trans_final.kode_faskes_baru
		FROM data_pm  LEFT JOIN propinsi ON data_pm.id_prov_pm = propinsi.id_prop 
		LEFT JOIN kota c ON data_pm.id_kota_pm = c.id_kota 
		LEFT JOIN trans_final ON trans_final.id_faskes=data_pm.id_faskes  
		WHERE 1=1 ".$raw_user_id." ");
		return $sql->result_array();
	}
...
ORDER BY data_labkes.id_kota ASC, registrasi_user.id ASC");	
	...
	function getdatarmepm($user_id=NULL)
	{
			$raw_user_id=" and id_faskes='".$user_id."' ";

		$sql = $this->db->query("SELECT * FROM data_rme  WHERE 1=1 ".$raw_user_id." ");
		return $sql->result_array();
	}
	function getdatapcarepm($user_id=NULL)
	{
			$raw_user_id=" and id_faskes='".$user_id."' ";

		$sql = $this->db->query("SELECT * FROM bridging_pcare  WHERE 1=1 ".$raw_user_id." ");
		return $sql->result_array();
	}
	
	function getdataalkespm($user_id=NULL)
	{
			$raw_user_id=" and trans_alkes_pm.id_faskes='".$user_id."' ";

		$sql = $this->db->query("SELECT trans_alkes_pm.* FROM trans_alkes_pm  WHERE 1=1 ".$raw_user_id." ");
		return $sql->result_array();
	}

	function getdataobatpm($user_id=NULL)
	{
			$raw_user_id=" and trans_obat_pm.id_faskes='".$user_id."' ";

		$sql = $this->db->query("SELECT trans_obat_pm.* FROM trans_obat_pm  WHERE 1=1 ".$raw_user_id." ");
		return $sql->result_array();
	}

	function getdatavendor($vendor_baru)
	{
			
		$sql = $this->db->query("SELECT dtoId, nameFacility FROM sim_pengembang  WHERE 1=1 AND id = '".$vendor_baru."' ");
		return $sql->result_array();
	}

	function getdataobatnewpm($user_id=NULL)
	{
			$raw_user_id=" and a.id_faskes='".$user_id."' ";

		$sql = $this->db->query("SELECT a.id, a.no_batch, a.nama_perusahaan, b.nama AS obat_id, c.nama AS jenis_sediaan_id,
									d.nama AS sumber_pembelian, b.id as id_obat
									FROM trans_obat_new_pm a 
									JOIN m_obat b ON b.id=a.obat_id
									JOIN m_jenis_sediaan c ON c.id=a.jenis_sediaan_id
									JOIN m_sumber_pembelian_obat d ON d.id=a.sumber_pembelian
								WHERE 1=1 ".$raw_user_id." ");
		return $sql->result_array();
	}
