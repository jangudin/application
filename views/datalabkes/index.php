...
<section class="content">
		...
		<div class="col-md-12">
			<div class="nav-tabs-custom">
				...
				<ul class="nav nav-tabs">
					<li class="active"><a href="<?php echo base_url('labkes/inputan_data_faskes_labkes'); ?>">Data Dasar</a></li>
										...
										<form role="form" method="POST" action="" enctype='multipart/form-data'>
											<div class="box-body">
												...
												<div class="form-group">
													...
													<div class="col-sm-5">
														<?= form_dropdown('jenis_lab', dropdown_jenis_lab($data[0]['jenis_pelayanan']), $data[0]['jenis_lab'], 'id="jenis_lab" onchange="openshowjenispelayanan(this.value)" class="form-control select2" required'); ?>
													</div>
													<div style="clear:both;"></div>
												</div>
												<?php
    ...
    function initialize(){
       ...
       addMarker(<? //= (!empty($data[0]['latitude']) ? $data[0]['latitude'] : '-6.2022'); 
					?>, <? //= (!empty($data[0]['latitude']) ? $data[0]['longitude'] : '106.8831'); 
						?>,"Posisi");
...
<!--	<script type="text/javascript">
	...
	function initMap() {
		...
		var centerCoordinates = new google.maps.LatLng(<? //= (!empty($data[0]['latitude']) ? $data[0]['latitude'] : ''); 
														?>, <? //= (!empty($data[0]['latitude']) ? $data[0]['longitude'] : '106.8831'); 
															?>);
		...
		if ("geolocation" in navigator){
			navigator.geolocation.getCurrentPosition(function(position){ 
				...
				document.getElementById("tampil").innerHTML = infoWindowHTML; 
												...
												<div class="form-group">
													<label class="col-sm-2 control-label">HOTLINE LABORATORIUM MEDIS</label>
													<div class="col-sm-5">
												...
												<div class="form-group">
													...
													<div class="col-sm-5">
														<input type="upload_surat_permohonan_kode_lab_medis" name="upload_surat_permohonan_kode_lab_medis" value="<?= (empty($data[0]['upload_surat_permohonan_kode_lab_medis']) ? '' : $data[0]['upload_surat_permohonan_kode_lab_medis']) ?>" <?= $auth_wajib_surat_izin ?> class="form-control" autocomplete="off">
														<?php
														if (!empty($data[0]['upload_surat_permohonan_kode_lab_medis'])) {
												...
												<div class="form-group">
													...
													<div class="col-sm-5">
														<input type="upload_surat_izin_operasional_lab_medis" name="upload_surat_izin_operasional_lab_medis" value="<?= (empty($data[0]['upload_surat_izin_operasional_lab_medis']) ? '' : $data[0]['upload_surat_izin_operasional_lab_medis']) ?>" <?= $auth_wajib_surat_izin_operasional ?> class="form-control" autocomplete="off">
														<?php
														if (!empty($data[0]['upload_surat_izin_operasional_lab_medis'])) {
												...
												<div class="form-group">
													...
													<div class="col-sm-5">
														<input type="upload_visi_misi" name="upload_visi_misi" value="<?= (empty($data[0]['upload_visi_misi']) ? '' : $data[0]['upload_visi_misi']) ?>" <?= $auth_wajib_visi_misi ?> class="form-control" autocomplete="off">
														<?php
														if (!empty($data[0]['upload_visi_misi'])) {
												...
												<div class="form-group">
													...
													<div class="col-sm-5">
														<input type="upload_struktur_organisasi" name="upload_struktur_organisasi" value="<?= (empty($data[0]['upload_struktur_organisasi']) ? '' : $data[0]['upload_struktur_organisasi']) ?>" <?= $auth_wajib_struktur_organisasi ?> class="form-control" autocomplete="off">
														<?php
														if (!empty($data[0]['upload_struktur_organisasi'])) {
												...
												<div class="form-group" id="dokumen_sertifikat_akreditasi_html" style="display:<?= $var_style_status_akreditasi ?>">
													<label class="col-sm-2 control-label">Link Drive Dokumen Sertifikat Akreditasi</label>
													<div class="col-sm-5">
														<input type="upload_dokumen_sertifikat_dokumen" name="upload_dokumen_sertifikat_dokumen" value="<?= (empty($data[0]['upload_dokumen_sertifikat_dokumen']) ? '' : $data[0]['upload_dokumen_sertifikat_dokumen']) ?>" <?= $auth_wajib_dokumen_sertifikat_dokumen ?> class="form-control" autocomplete="off">
													</div>
													<div style="clear:both;"></div>
													...
													<div class="col-sm-5">
														<input type="file" name="upload_dokumen_sertifikat_dokumen" id="upload_dokumen_sertifikat_dokumen" <? //= $auth_wajib_dokumen_sertifikat_dokumen 
																																							?>>
														<a target="_blank" href="<?php //echo base_url('assets/uploads/berkas_operasional/' . $data[0]['upload_dokumen_sertifikat_dokumen']); 
																					?>"><?php //echo $data[0]['upload_dokumen_sertifikat_dokumen']; 
																						?></a>
														<input type="hidden" name="old_dokumen_sertifikat_dokumen" value="<? //= $data[0]['upload_dokumen_sertifikat_dokumen'] 
																															?>" id="old_dokumen_sertifikat_dokumen">
													</div>/* Hanya File PDF Kurang Dari 2 MB
													<div style="clear:both;"></div> -->
												</div>
...
<script>
	...
	$('[name="jenis_pelayanan"]').change(function() {
		$('#jenis_lab').val('');
		$.ajax({
			url: "<?php echo site_url('labkes/dropdownLabkesJenispelayanan') ?>/" + $('#jenis_pelayanan').val().replace(/ /g, "_"),
			dataType: "json",
			type: "GET",
			success: function(data) { //
