...
?>
            <ul class="nav nav-tabs">
				...
				<li ><a href="<?php echo base_url('utd/verifikasi_pengajuan_faskes_sdm_utd/').$user_id;?>">Data SDM</a></li>
				 <li  class="active" ><a href="<?php echo base_url('utd/verifikasi_pengajuan_faskes_sarpras_alkes_utd/').$user_id;?>">Data Sarpras</a></li>
     ...
     <section class="content">
      ...
      <div class="row">
		...
		<?php
			$no=0;
			foreach(dropdown_sarpras_alkes_utd($user[0]['jenis_utd']) as $key => $value){
			$no++;
			...
			?>
			 ...
			 <tr>
			 <td><?=$no;?></td>
			 <td><?php 	foreach(dropdown_sarpras_alkes_utd_type($key) as $key3 => $value3){ echo $value3; } ?></td>
			  <td><?php 	foreach(dropdown_sarpras_alkes_utd_sub_type($key) as $key3 => $value3){ echo $value3; } ?></td>
			 <td><?=$value?></td>
			<td>
			<input type="checkbox" name="isian[<?=$key;?>]" <?=($isian =='Ada' ? 'checked' : '')?>></td>	 
			<!--   <td width="30%"><?php 	foreach(dropdown_sarpras_alkes_utd_sub_keterangan($key) as $key3 => $value3){ echo $value3; } ?></td>-->
