    	...
    	<div class="box">
    		<div class="box-content">
			  <ul id="myTab" class="nav nav-tabs">
                  <li class="active"><a href="<?php echo base_url('');?>">List User Yang Mendaftar</a></li> 
              </ul>
			    <div id="myTabContent" class="tab-content">
			 <div class="tab-pane fade in active">
			 <br>
    			<h3 class="page-header">LIST USER YANG MENDAFTAR</h3>
<!--
...
$no=0;
		...
		?>
				...
				<td><?php echo $no; ?></td>
                  <td><?php echo $value['email']; ?></td>
                  <td><?php echo $value['nama_lengkap']; ?></td>
				 <td><?php echo $value['kategori_user']." (".$value['keterangan'].")"; ?></td>
				  <td><?php echo "PUSKESMAS ".$value['nama_fasyankes']; ?></td>
				   <td><?php echo $value['kode_baru']; ?></td>
                   <td><?php echo form_dropdown('status_login', dropdown_status_login(), $value['validate'],'id="status_login" class="form-control select2" disabled'); ?></td>
                  <td><?php if($this->session->userdata('id_kategori') !='2'){ ?><a href="<?=site_url('dashboard/verifikasi_pendaftaran_faskes_puskesmas/'.$value['id']);?>"><button type="button" class="btn btn-default"><span class="glyphicon glyphicon-zoom-in" aria-hidden="true"></span> Verifikasi</button></a><?php } ?><?php if($value['jml']=='0' && $this->session->userdata('id_kategori') =='3'){ ?> | <a onclick="return confirm('Apakah Anda Yakin?');" href="<?=site_url('dashboard/hapus_pendaftaran_faskes_puskesmas/'.$value['id']);?>"><button type="button" class="btn btn-default"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span> Hapus</button></a><?php } ?></td>
                </tr>
            <?php } ?> 
                </tbody>
