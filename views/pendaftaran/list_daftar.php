		...
		?>
				...
				<td><?php echo $no; ?></td>
				 ...
				 <td><?php echo $value['kategori_user']." (".$value['keterangan'].")"; ?></td>
                  ...
                  <td><?php if($this->session->userdata('id_kategori') !='2'){ ?><a href="<?=site_url('dashboard/verifikasi_pendaftaran_faskes/'.$value['id']);?>"><button type="button" class="btn btn-default"><span class="glyphicon glyphicon-zoom-in" aria-hidden="true"></span> Verifikasi</button></a><?php } ?><?php if($value['jml']=='0' && $this->session->userdata('id_kategori') =='3' && $value['validate'] != '2'){ ?> | <a onclick="return confirm('Apakah Anda Yakin?');" href="<?=site_url('dashboard/hapus_pendaftaran_faskes/'.$value['id']);?>"><button type="button" class="btn btn-default"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span> Hapus</button></a><?php } ?></td>
                </tr>
            <?php } ?> 
                </tbody>
