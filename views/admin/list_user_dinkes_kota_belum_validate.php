	...
	<div class="box">
		<div class="box-content">
			<ul id="myTab" class="nav nav-tabs">
				<li><a href="<?php echo base_url('dashboard/list_user_dinkes_kota'); ?>">List User Dinkes Kab/Kota</a></li>
				<li class="active"><a href="<?php echo base_url('dashboard/list_user_dinkes_kota_belum_validate'); ?>">List User Dinkes Kab/Kota(Belum Validasi)</a></li>
			</ul>
			<div id="myTabContent" class="tab-content">
				<div class="tab-pane fade in active">
					<br>
					<h3 class="page-header">LIST USER DINKES KAB/KOTA BELUM DI VALIDASI</h3>
												...
												<div class="row">
													<table id="example1" class="table table-bordered table-striped">
														...
														<tbody>
															...
															?>
																<tr>
																	<td><?= html_escape($value['id']); ?></td>
																	<td><?= html_escape($value['email']); ?></td>
																	<td><?= html_escape($value['type_user']); ?></td>
																	<td><?= html_escape($value['nama_lengkap']); ?></td>
																	<td><?= html_escape($value['jabatan']); ?></td>
																	<td><?= html_escape($value['no_hp']); ?></td>
																	<td><?= html_escape($value['nama_prop']); ?></td>
																	<td><?= html_escape($value['nama_kota']); ?></td>
																	<td>
																		<?php if ($this->session->userdata('id_kategori') != 1): ?>
																			<a onclick="return confirm('Yakin DI Validasi?');" href="<?= site_url('dashboard/tambah_user_dinkes_kota_belum_validasi/' . $value['id_faskes']); ?>">
																				<button type="button" class="btn btn-default">
																					<span class="glyphicon glyphicon-ok" aria-hidden="true"></span> Validasi
																				</button>
																			</a>
																		<?php endif; ?>
																		...
																		<?php if ($this->session->userdata('id_kategori') == 2): ?>
																			|
																			<a onclick="return confirm('Apakah Anda Yakin?');" href="<?= site_url('dashboard/hapus_user_dinkes_kota/' . $value['id_faskes'] . '/list_user_dinkes_kota_belum_validate'); ?>">
																				<button type="button" class="btn btn-default">
