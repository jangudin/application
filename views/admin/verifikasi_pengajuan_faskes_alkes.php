...
?>
            ...
            <li ><a href="<?php echo base_url('dashboard/verifikasi_pengajuan_faskes/').$this->encrypt->encode($user_id);?>">Data Dasar</a></li>	  
			   <li class="active"><a href="<?php echo base_url('dashboard/verifikasi_pengajuan_faskes_alkes/').$this->encrypt->encode($user_id);?>">Data Sarpras</a></li>
    ...
    <section class="content">
      ...
      <div class="row">
		...
		<!--	  <td><?php foreach(dropdown_sarpras_alkes_klinik_type_bangunan($key) as $key2 => $value2){ echo $value2;  }?></td>-->
			 ...
			 ?>
			 <td>
			<?=($value2['is_checked']=='1' ? 'Ada' : 'Tidak Ada')?> 
			 <?php foreach(dropdown_sarpras_alkes_klinik_sub_keterangan($key) as $sub_keterangan => $valuesub_keterangan){ echo ($valuesub_keterangan=='Ada' ? '/ '.$value2['sub_keterangan'].'' : '');  }?>
			...
			?>
			 	 <td>
				  <input type="checkbox" name="is_checked[<?=$key;?>]"  <?php foreach(dropdown_sarpras_alkes_klinik_auth($key) as $auth => $valueauth){ echo ($valueauth=='wajib ada' ? 'required' : '');  }?>   value="1">
				   <?php foreach(dropdown_sarpras_alkes_klinik_sub_keterangan($key) as $sub_keterangan => $valuesub_keterangan){ echo ($valuesub_keterangan=='Ada' ? '' : '');  }?>
