...
?>
            <ul class="nav nav-tabs">
			  ...
			  <li  ><a href="<?php echo base_url('dashboard/inputan_data_sdm');?>">Data SDM</a></li>
			   <li class="active" ><a href="<?php echo base_url('dashboard/selesaikan');?>">Selesaikan</a></li>
            </ul>
        ...
        <div class="col-md-12" >
          ...
          <div class="box box-primary">
			...
			if(empty($data[0]['final'])){
			 ...
			 <input type="hidden" name="pma" value="<?=$getdataklinik[0]['jenis_modal_usaha']?>">
			  <input type="hidden" name="id_prov" value="<?=$user[0]['id_prov']?>">
			  <input type="hidden" name="id_kota" value="<?=$user[0]['id_kota']?>">
			   <input type="hidden" name="kode_regional" value="<?=$getdataklinik[0]['kode_regional']?>">
			  
			 <button type="submit" name="submit" id="submit"  class="btn btn-primary">Kirim</button></td>
			 </tr>
			  <tr>
			 <td colspan="2"><b>Kode Fasyankes Lama, Bila Ada :</b> <input type="text" name="kode_faskes_lama" value="<?=$data[0]['kode_faskes_lama']?>">
			 </td>
			 </tr>
			...
			if(!empty($data[0]['token_kode_faskes']) && empty($data[0]['kode_faskes_baru'])){
			?>
			<!-- <tr>
			...
			<?php
			}else if(!empty($data[0]['kode_faskes_baru'])){
			?>
			 <tr>
			 <td><b>Kode Faskes : <?=$data[0]['kode_faskes_baru']?></b></td>
			 <td></td>
			 </tr>
			<?php
			}else{
