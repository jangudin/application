<?php
class Labkes_dev extends CI_Controller{
	...
	function verifikasikan_kirim_labkes($id=NULL){
		    ...
		    $post = $this->input->post();
				...
				if(isset($post['submit_validasi'])){
					...
					$where2 = array(
                     ...
                     if($post["jenis_pratama_utama"]=='Provinsi'){
					 ...
					 }else if($post["jenis_pratama_utama"]=='Kesmas'){
					 ...
					 }else if($post["jenis_pratama_utama"]=='Yankes'){
						$datas_detail = array('id_validate_kota' =>$this->session->userdata('user_id'),'status_validasi_kota'=>'Sudah Validasi','status_validasi_kemkes'=>'Belum Validasi');
					 }
			...
			/* 	if(isset($post['submit_setujui'])){
				...
				//$token = substr(str_shuffle(str_repeat("0123456789abcdefghijklmnopqrstuvwxyz", 5)), 0, 50);
					...
					if($post["jenis_pratama_utama"]=='Provinsi'){
						$datas_detail = array('final' =>0,'catatan'=>$post["catatan"],'token_kode_faskes'=>'','id_validate_kota' =>'','status_validasi_kota'=>'Perbaikan','status_validasi_prov'=>'');
					 }else if($post["jenis_pratama_utama"]=='Kesmas'){
						$datas_detail = array('final' =>0,'catatan'=>$post["catatan"],'token_kode_faskes'=>'','id_validate_kota' =>'','status_validasi_kota'=>'Perbaikan','status_validasi_kemkes'=>'');
					 }else if($post["jenis_pratama_utama"]=='Yankes'){
						$datas_detail = array('final' =>0,'catatan'=>$post["catatan"],'token_kode_faskes'=>'','id_validate_kota' =>'','status_validasi_kota'=>'Perbaikan','status_validasi_kemkes'=>'');
					 }
		...
		$mail = $this->send_email("registrasifasyankes@kemkes.go.id","Sekretariat Direktorat Jenderal Pelayanan Kesehatan","uLH0%RYL",$title,$email,$data['data'][0]['nama_lengkap'],$message);
				...
				//$token = substr(str_shuffle(str_repeat("0123456789abcdefghijklmnopqrstuvwxyz", 5)), 0, 50);
						...
						$datas_detail = array('id_validate_kota' =>'','catatan'=>$post["catatan"],'status_validasi_kota' =>'Belum Validasi','status_validasi_prov' =>'Perbaikan');
			...
			$this->registrasiusermodel->input_data('timeline',$datas_log);
				...
				//$token = substr(str_shuffle(str_repeat("0123456789abcdefghijklmnopqrstuvwxyz", 5)), 0, 50);
						...
						$datas_detail = array('id_validate_kota' =>'','catatan'=>$post["catatan"],'status_validasi_kota' =>'Belum Validasi','status_validasi_kemkes' =>'Perbaikan');
	...
	public function inputan_data_faskes_labkes()
	{
		...
		if(!empty($_FILES["upload_surat_permohonan_kode_lab_medis"]["name"])){
			if(in_array($type1, array("pdf"))) {
				...
				redirect('labkes_dev/inputan_data_faskes_labkes');
		...
		$datas = array(
				...
				'jenis_pelayanan_child' =>$post['jenis_pelayanan_child'],
			...
			$this->template->utama('datalabkes/index_dev',$data);  
	}
	
	public function inputan_standar_pelayanan()
	{
	
		$where = array('id_faskes' => $this->session->userdata('user_id'));
		$data['data2']= $this->registrasiusermodel->select_data('trans_final',$where)->result_array();
		$data['data']= $this->registrasiusermodel->getdatastandarpelayananlabkes($this->session->userdata('user_id'));	
		...
		$post = $this->input->post();
			...
			$this->registrasiusermodel->delete_data('trans_labkes_standar_pelayanan',$where2);

			foreach($post['id_sarpras_alkes'] as $ids){

				$datas_detail = array(
			    'id_faskes' =>$post['id_faskes'],
				'id_standar_pelayanan' => $ids,
				'isian' => $post['isian'][$ids],
				'keterangan' => $post['keterangan'][$ids]
				);
				
		
			$this->registrasiusermodel->input_data('trans_labkes_standar_pelayanan',$datas_detail);
//echo $this->db->last_query();
//exit();
			}
			...
			$this->session->set_flashdata('icon_name', 'check');
			$this->session->set_flashdata('message_name', 'Sukses Input Data!');
			redirect('labkes_dev/inputan_standar_pelayanan/');
		}
			
			$this->template->utama('datalabkes/index_standar_pelayanan_labkes_dev',$data);  
	}
...
//exit();
			...
			$this->session->set_flashdata('icon_name', 'check');
			$this->session->set_flashdata('message_name', 'Sukses Input Data!');
			redirect('labkes_dev/inputan_data_sarpras_alkes_labkes/');
		}
			
			$this->template->utama('datalabkes/index_sarpras_alkes_labkes_dev',$data);  
	}
	
	public function inputan_data_sdm_labkesmas()
	{
	
		$where = array('id_faskes' => $this->session->userdata('user_id'));
		$data['data2']= $this->registrasiusermodel->select_data('trans_final',$where)->result_array();
		$data['data']= $this->registrasiusermodel->getdatasdmlabkesmas($this->session->userdata('user_id'));	
		$data['user']= $this->registrasiusermodel->getdatalabkes($this->session->userdata('user_id'));
		$data['user_id']= $this->session->userdata('user_id');
		$post = $this->input->post();
			...
			$this->registrasiusermodel->delete_data('trans_labkes_sdm_kesmas',$where2);
			...
			$this->registrasiusermodel->input_data('trans_labkes_sdm_kesmas',$datas_detail);
//echo $this->db->last_query();
//exit();
			}
			...
			$this->session->set_flashdata('icon_name', 'check');
			$this->session->set_flashdata('message_name', 'Sukses Input Data!');
			redirect('labkes_dev/inputan_data_sdm_kesmas/');
		}
			
			$this->template->utama('datalabkes/index_sdm_kesmas_dev',$data);  
	}
	...
	$where = array(
		...
		//$show_user = $this->loginmodel->cek_login("registrasi_user",$where)->result_array();
			...
			$where_edit = array(
				...
				$datas = array(  
				  'validate_token' => 1,
				  'kode_faskes' =>$this->registrasiusermodel->findNoFaskes($kode_regional_link,'7'),
		...
		function send_email($emaildari,$namadari,$password,$subjek,$emailtujuan,$namatujuan,$pesan)
 {
	...
	public function dropdownLabkesJenispelayanan($jenis=null,$filters='') {
		...
		//if ($this->input->is_ajax_request()) {
			...
			$rsData = $this->registrasiusermodel->get_labkes_jenis_pelayanan_dev($filters, $order);//exit(show_last_query());
			echo json_encode($rsData);
		//}
		return;
	}
	
	public function dropdownLabkesJenispelayananChild($jenis=null,$filters='') {
		...
		//if ($this->input->is_ajax_request()) {
			...
			$rsData = $this->registrasiusermodel->get_labkes_jenis_pelayanan_child($filters, $order);//exit(show_last_query());
			echo json_encode($rsData);
		//}
		return;
	}
