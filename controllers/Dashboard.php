...
class Dashboard extends CI_Controller
{
	...
	function list_user_yang_mendaftar_puskesmas($id = 0)
	{
		$data['data']['query'] = $this->registrasiusermodel->getlistpendaftaranpuskesmas($this->session->userdata('id_kategori'), $this->session->userdata('user_id'), $this->session->userdata('id_kota'), $this->session->userdata('id_prov'));
		$this->template->utama('pendaftaran/list_daftar_puskesmas', $data);
	}
	...
	function verifikasi_pendaftaran_faskes_puskesmas($id = NULL)
	{
		...
		$this->template->utama('pendaftaran/verifikasi_pendaftaran_faskes_puskesmas', $data);
	}
	...
	function validate_kirim_email($id = NULL)
	{
		...
		$input_SSO = $this->create_sso_akun($id);
		...
		// $this->session->set_flashdata('Error_name', 'Validasi');

		// redirect('dashboard/verifikasi_pendaftaran_faskes/' . $id, 'refresh');
	}

	function create_sso_akun($id = NULL)
	{
		$getData = $this->registrasiusermodel->getbylistpendaftaran($id);

		$categoryId = $getData[0]['id_kategori'];
		$email = $getData[0]['email'];
		$password = $getData[0]['kata_sandi'];
		$fullName = $getData[0]['nama_lengkap'];
		$companyName = $getData[0]['nama_fasyankes'];
		...
		curl_setopt_array($curl, array(
			CURLOPT_URL => 'https://akun-yankes.kemkes.go.id/sso/v1/loginadmin',
		...
		if ($status_code == 201) {
			...
			curl_setopt_array($curl2, array(
				CURLOPT_URL => 'https://akun-yankes.kemkes.go.id/sso/v1/user',
				CURLOPT_RETURNTRANSFER => true,
				CURLOPT_ENCODING => '',
				CURLOPT_MAXREDIRS => 10,
				CURLOPT_TIMEOUT => 0,
			...
			if ($status_code == 201) {
				$this->session->set_flashdata('kode_name', 'alert alert-success alert-dismissible');
				$this->session->set_flashdata('icon_name', 'success');
				$this->session->set_flashdata('message_name', 'Akun Berhasil di Verifikasi.');
				$this->session->set_flashdata('Error_name', '');
				return 1;
			} else if ($status_code == 422) {
				$this->session->set_flashdata('kode_name', 'alert alert-danger alert-dismissible');
				$this->session->set_flashdata('icon_name', 'danger');
				$this->session->set_flashdata('message_name', 'Terjadi Kesalahan Silahkan Menghubungi Admin!');
				$this->session->set_flashdata('Error_name', 'Kode : 422');
				return 0;
			} else if ($status_code == 404) {
				$data_response = json_decode($response2, true);
				$message = $data_response['message'];
				$this->session->set_flashdata('kode_name', 'alert alert-danger alert-dismissible');
				$this->session->set_flashdata('icon_name', 'danger');
				$this->session->set_flashdata('message_name', 'Terjadi Kesalahan Silahkan Menghubungi Admin!');
				$this->session->set_flashdata('Error_name', 'Kode : ' . $status_code . ', Message : ' . $message);
				return 0;
			} else {
				$this->session->set_flashdata('kode_name', 'alert alert-danger alert-dismissible');
				$this->session->set_flashdata('icon_name', 'danger');
				$this->session->set_flashdata('message_name', 'Terjadi Kesalahan Silahkan Menghubungi Admin!');
				$this->session->set_flashdata('Error_name', 'Kode : ' . $status_code);
				return 0;
			}
		} else {
			$this->session->set_flashdata('kode_name', 'alert alert-danger alert-dismissible');
	...
	public function inputan_data_faskes()
	{
		...
		if (isset($post['submit'])) {
			...
			// $url_operasional = base_url("/assets/uploads/berkas_operasional/" . $filename1);
			...
			// $url_registrasi = base_url("/assets/uploads/berkas_operasional/" . $filename2);
			...
			// $url_penanaman_modal = base_url("/assets/uploads/berkas_operasional/" . $filename3);

			// if (!empty($_FILES["operasional"]["name"])) {
			// 	if (in_array($type1, array("pdf"))) {
			// 		$file_content = file_get_contents($_FILES["operasional"]["tmp_name"]);
			...
			// 		$file_content = file_get_contents($_FILES["dokumen_registrasi"]["tmp_name"]);
			// 		if (strpos($file_content, '<?php') !== false) {
			// 			// die("File contains PHP code.");
			// 			$this->session->set_flashdata('kode_name', 'alert alert-danger alert-dismissible');
			// 			$this->session->set_flashdata('icon_name', 'warning');
			...
			// 		$file_content = file_get_contents($_FILES["bukti_penanaman_modal_asing"]["tmp_name"]);
			// 		if (strpos($file_content, '<?php') !== false) {
			// 			// die("File contains PHP code.");
			// 			$this->session->set_flashdata('kode_name', 'alert alert-danger alert-dismissible');
			// 			$this->session->set_flashdata('icon_name', 'warning');
			...
			//exit();
			if (!empty($post['nama_klinik'])) {
				$nama_klinik = $post['nama_klinik'];
			} else {
				$nama_klinik = $post['nama_klinik_old'];
			}
			$datas = array(
				...
				'url_dokumen_operasional' => $post['operasional'],
				'url_dokumen_registrasi' => $post['dokumen_registrasi'],
				'url_dokumen_penanaman_modal_asing' => $post['bukti_penanaman_modal_asing']
			);
		...
		if (isset($post['submit'])) {
			...
			if (!empty($_FILES["upload_surat_permohonan_kode_lab_medis"]["name"])) {
				if (in_array($type1, array("pdf"))) {
					...
					$file_content = file_get_contents($_FILES["upload_surat_permohonan_kode_lab_medis"]["tmp_name"]);
					if (strpos($file_content, '<?php') !== false) {
						// die("File contains PHP code.");
						$this->session->set_flashdata('kode_name', 'alert alert-danger alert-dismissible');
						$this->session->set_flashdata('icon_name', 'warning');
			...
			if (!empty($_FILES["upload_surat_izin_operasional_lab_medis"]["name"])) {
				if (in_array($type2, array("pdf"))) {
					$file_content = file_get_contents($_FILES["upload_surat_izin_operasional_lab_medis"]["tmp_name"]);
					if (strpos($file_content, '<?php') !== false) {
						// die("File contains PHP code.");
						$this->session->set_flashdata('kode_name', 'alert alert-danger alert-dismissible');
						$this->session->set_flashdata('icon_name', 'warning');
			...
			if (!empty($_FILES["upload_visi_misi"]["name"])) {
				if (in_array($type3, array("pdf"))) {
					$file_content = file_get_contents($_FILES["upload_visi_misi"]["tmp_name"]);
					if (strpos($file_content, '<?php') !== false) {
						// die("File contains PHP code.");
						$this->session->set_flashdata('kode_name', 'alert alert-danger alert-dismissible');
						$this->session->set_flashdata('icon_name', 'warning');
			...
			if (!empty($_FILES["upload_struktur_organisasi"]["name"])) {
				if (in_array($type4, array("pdf"))) {
					...
					$file_content = file_get_contents($_FILES["upload_struktur_organisasi"]["tmp_name"]);
					if (strpos($file_content, '<?php') !== false) {
						// die("File contains PHP code.");
						$this->session->set_flashdata('kode_name', 'alert alert-danger alert-dismissible');
						$this->session->set_flashdata('icon_name', 'warning');
			...
			if (!empty($_FILES["upload_dokumen_sertifikat_dokumen"]["name"])) {
				if (in_array($type5, array("pdf"))) {
					...
					$file_content = file_get_contents($_FILES["upload_dokumen_sertifikat_dokumen"]["tmp_name"]);
					if (strpos($file_content, '<?php') !== false) {
						// die("File contains PHP code.");
						$this->session->set_flashdata('kode_name', 'alert alert-danger alert-dismissible');
						$this->session->set_flashdata('icon_name', 'warning');
	...
	function list_user_yang_mengajukan_tes($id = 0)
	{
		$data['data']['query'] = $this->registrasiusermodel->getlistpengajuan($this->session->userdata('id_kategori'), $this->session->userdata('id_kota'));
		$this->template->utama('admin/list_user_yang_mengajukan_tes', $data);
	}

	function server_side_list_yang_mengajukan()
	{
		...
		$datas['count'] = $this->registrasiusermodel->getlistpengajuantescount($this->session->userdata('id_kategori'), $this->session->userdata('id_kota'));

		$datas['data'] = $this->registrasiusermodel->getlistpengajuantes($this->session->userdata('id_kategori'), $this->session->userdata('id_kota'), $start, $end, $search);
		$data = array();
		$no = 0;
		foreach ($datas['data'] as $key => $value) {
			...
			$no++;
			$row = array();
			$row[] = $value['kode_faskes'];
			$row[] = $value['nama_klinik'];
			$row[] = $value['persalinan'];
			$row[] = (!empty($value['token_kode_faskes']) ? 'Sudah Di Validasi' : 'Belum Di Validasi');
			$row[] = $value['status_klinik'];
			$row[] = $value['status_klinik'];
			$row[] = '';
			$data[] = $row;
		}
		...
		$output = array(
			"draw" => 6,
			"recordsTotal" => $datas['count'][0]['jml'],
			"recordsFiltered" => $datas['count'][0]['jml'],
			"data" => $data,
		);
	...
	function ubah_status_klinik($idx = 0, $jenis = NULL)
	{
		//$data['data']['query'] = $this->registrasiusermodel->getlistpengajuan($this->session->userdata('id_kategori'),$this->session->userdata('id_kota'));
		...
		if (isset($post['submit'])) {
			...
			$datas_detail2 = array(
				'id_faskes' => $post["id_faskes"],
				'status' => $post["status_klinik"],
				'alasan_status' => $post["alasan_status_klinik"],
				'jenis_table' => 'data_klinik'

			);

			$this->registrasiusermodel->input_data('log_ubah_status', $datas_detail2);
			if (!empty($jenis)) {
				redirect('dashboard/list_user_yang_mengajukan_pma');
			} else {
	...
	function verifikasi_pengajuan_faskes($idx = NULL)
	{
		$id=$this->encrypt->decode($idx);
		$post = $this->input->post();
		if (isset($post['submit'])) {
	...
	function verifikasi_pengajuan_faskes_alkes($idx = NULL)
	{
	...
	function verifikasi_pengajuan_faskes_sdm($idx = NULL)
	{
		$id=$this->encrypt->decode($idx);
		$post = $this->input->post();
	...
	function verifikasikan_kirim($idx = NULL)
	{
		$id=$this->encrypt->decode($idx);
		$post = $this->input->post();
		if (isset($post['submit'])) {
	...
	public function tambah_master_data_sdm($idx = null)
	{
		$id=$this->encrypt->decode($idx);
		$this->load->model('master/Sdmmodel');
	...
	public function hapus_master_data_sdm($idx = null)
	{
		$id=$this->encrypt->decode($idx);
		$this->load->model('master/Sdmmodel');
	...
	public function tambah_master_data_sarpras($idx = null)
	{
		$id=$this->encrypt->decode($idx);
		$this->load->model('master/Sarprasmodel');
	...
	public function hapus_master_data_sarpras($idx = null)
	{
		$id=$this->encrypt->decode($idx);
		$this->load->model('master/Sarprasmodel');
	...
	public function tambah_master_data_sarpras_utd($idx = null)
	{
		$id=$this->encrypt->decode($idx);
		$this->load->model('master/Sarprasutdmodel');
	...
	public function hapus_master_data_sarpras_utd($idx = null)
	{
		$id=$this->encrypt->decode($idx);
		$this->load->model('master/Sarprasutdmodel');
	...
	public function tambah_master_data_alkes_utd($idx = null)
	{
		$id=$this->encrypt->decode($idx);
		$this->load->model('master/Alkesutdmodel');
	...
	public function hapus_master_data_alkes_utd($idx = null)
	{
		$id=$this->encrypt->decode($idx);
		$this->load->model('master/Alkesutdmodel');
	...
	function list_pcare()
	{
		...
		$this->template->utama('admin/list_pcare', $data);
		// echo "tes";
	}

	function timeline($idx = NULL)
	{
		$id=$this->encrypt->decode($idx);
	...
	function rekap_data($id = NULL)
	{
		...
		if (isset($_POST['cari']) && ($this->session->userdata('id_kategori') == 1 || $this->session->userdata('id_kategori') == 8 || $this->session->userdata('id_kategori') == 10)) {
			...
			$data['data']['query'] = $this->registrasiusermodel->getrekap_data(str_replace('/', '-', $_POST['tgl1']), str_replace('/', '-', $_POST['tgl2']), $_POST['id_prov'], $_POST['id_kota'], '', $_POST['jenis_klinik'], $_POST['jenis_perawatan'], $_POST['sorting'], $_POST['type_sorting'], $_POST['layanan_klinik']);
			//var_dump($data['data']['query']);
			//	exit();
		}

		if ($this->session->userdata('id_kategori') == 3) {
			$data['data']['query'] = $this->registrasiusermodel->getrekap_data('', '', '', $this->session->userdata('id_kota'), '', $_POST['jenis_klinik'], $_POST['jenis_perawatan'], $_POST['sorting'], $_POST['type_sorting'], $_POST['layanan_klinik']);
		}

		if ($this->session->userdata('id_kategori') == 2) {
			$data['data']['query'] = $this->registrasiusermodel->getrekap_data('', '', $this->session->userdata('id_prov'), $_POST['id_kota'], '', $_POST['jenis_klinik'], $_POST['jenis_perawatan'], $_POST['sorting'], $_POST['type_sorting'], $_POST['layanan_klinik']);
		}
	...
	function rekap_data_target_blank($id = NULL)
	{
		...
		$this->load->view('admin/rekap_data_target_blank', $data);
	}
	...
	function rekap_data_pm_all($id = NULL)
	{
		...
		if (isset($_POST['cari']) && ($this->session->userdata('id_kategori') == 1 || $this->session->userdata('id_kategori') == 8 || $this->session->userdata('id_kategori') == 10)) {
			...
			$data['data']['query'] = $this->registrasiusermodel->getrekap_data_pm('', '', $propinsi, $kota, $id_kategori);
		}
	...
	function rekap_pm_all_target_blank($id = NULL)
	{
		...
		$this->load->view('admin/rekap_pm_all_target_blank', $data);
	}
	...
	function monitoring_pm($id = NULL)
	{
		...
		$this->template->utama('admin/monitoring_pm', $data);
	}
	...
	public function edit_profile($idx = null)
	{
		...
		$hashed    = hash('sha256', $post['password_baru'] . $salt);
		...
		if (isset($post['simpan'])) {
			if ($post['password_baru'] == $post['confirm_password_baru']) {
				...
				if (!empty($post['id'])) {
					...
					$kirimAPI = $this->changePasswordSSO($dataAPI);
					if ($kirimAPI === 1) {
						$where = array(
							'id' => $post['id']
						);
	...
	public function edit_profile_new($idx = null)
	{
       $id=$this->encrypt->decode($idx);
		...
		if (isset($post['simpan'])) {
			...
			if (!empty($post['id'])) {
				...
				//$dbmutu= $this->load->database('inm', TRUE);			
				//$query = $dbmutu->query("update user set nama_lengkap='".$post['nama_lengkap']."' where username='".$post['kode']."'");
				/// $query2 = $dbmutu->query("update m_satker set nama='".$post['nama_lengkap']."' where kode_satker='".$post['kode']."'");
	...
	public function tambah_user_dinkes_kota($idx = null)
	{
		$id=$this->encrypt->decode($idx);
		$data['id'] = $id;
	...
	public function tambah_user_dinkes_propinsi($idx = null)
	{
		$id=$this->encrypt->decode($idx);
		$data['id'] = $id;
		$data['id_prov'] = $this->session->userdata('id_prov');
	...
	private function changePasswordSSO($data)
	{
		$curl = curl_init();
		curl_setopt_array($curl, array(
			CURLOPT_URL => 'https://akun-yankes.kemkes.go.id/sso/v1/login',
			CURLOPT_RETURNTRANSFER => true,
			CURLOPT_ENCODING => '',
			CURLOPT_MAXREDIRS => 10,
			CURLOPT_TIMEOUT => 0,
		...
		if ($status_code == 201) {
			...
			$curl2 = curl_init();
			curl_setopt_array($curl2, array(
				CURLOPT_URL => 'https://akun-yankes.kemkes.go.id/sso/v1/changepassword',
				CURLOPT_RETURNTRANSFER => true,
				CURLOPT_ENCODING => '',
				CURLOPT_MAXREDIRS => 10,
				CURLOPT_TIMEOUT => 0,
	...
	public function tambah_user_dinkes_kota_belum_validasi($id = null)
	{
		...
		if (!empty($id)) {
			...
			if ($input_SSO == 1) {
				$input = $this->registrasiusermodel->edit_data('registrasi_user', $where, $datas);
			}
	...
	public function tambah_user_dinkes_propinsi_belum_validasi($idx = null)
	{
		$id=$this->encrypt->decode($idx);
		if (!empty($id)) {
	...
	function validasi_link_kode_faskes($token, $id_link, $id, $kode_regional_link)
	{
		...
		if ($cek > 0) {
			...
			$datas = array(
				'validate_token' => 1,
				'kode_faskes' => $this->registrasiusermodel->findNoFaskesBaru($id_link, '5'),
				'kode_faskes_baru' => $this->registrasiusermodel->findNoFaskesBaru($id_link, '5'),
		...
		<br><br>
		Selamat datang di Aplikasi Registrasi Faskes Online.<br>
		Kode Faskes Anda : " . $data['trans_final'][0]['kode_faskes_baru'] . "
		<br><br>
		Jika ada pertanyaan lebih lanjut, silahkan menghubungi:<br>
		registrasi.fasyankes2@gmail.com<br>
		<br><br>
	...
	public function gedata_user_akses()
	{
		$this->load->model('master/Useraksesmodel');
		...
		$searchQuery = "";
		if ($searchValue != '') {
			$searchQuery = " (email like '%" . $searchValue . "%' or nama_fasyankes like '%" . $searchValue . "%' ) ";
		}

		## Total number of records without filtering
		$this->db->select('count(*) as allcount');
		$records = $this->db->get('registrasi_user')->result();
		...
		foreach ($records as $record) {
			...
			$data[] = array(
				...
				"nama_fasyankes" => $record->nama_fasyankes,
				"type_user" => $record->type_user,
				"tgl_buat_user" => $record->tgl_buat_user,
				"id" => $record->id
			);
		}
	...
	public function edit_user_akses($idx = null)
	{
		$id=$this->encrypt->decode($idx);
	...
	public function tambah_master_data_propinsi($idx = null)
	{
		$id=$this->encrypt->decode($idx);
		$this->load->model('master/Propinsimodel');
	...
	public function hapus_user_dinkes_propinsi($idx, $url)
	{
		$id=$this->encrypt->decode($idx);
		if (!empty($id)) {
			$where = array(
	...
	public function hapus_user_dinkes_kota($idx, $url)
	{
		$id=$this->encrypt->decode($idx);
	...
	public function hapus_pendaftaran_faskes()
	{
	...
	public function hapus_pendaftaran_faskes_puskesmas()
	{
		if (!empty($this->uri->segment(3))) {
		...
		redirect('dashboard/list_user_yang_mendaftar_puskesmas');
	}
	...
	function generate_kode_baru()
	{
		$post = $this->input->post();
		$data['data']['query'] = $this->registrasiusermodel->generate_kode_baru((isset($post['jenis']) ? $post['jenis'] : 5));
		$this->template->utama('admin/generate_kode_baru', $data);
	}
}
