...
class Pm extends CI_Controller
{
	...
	public function inputan_data_gambar_pm()
	{
		...
		if (isset($post['submit'])) {
			...
			$datas = array(
				...
				'keterangan' => "image",
				'id_faskes' => $this->session->userdata('user_id'),
				'url' =>$post['dokumen_gambar'],
				'tgl_upload' => date('Y-m-d H:i:s'),
				'url_full' => $post['dokumen_gambar'],
	...
	public function inputan_data_pm()
	{
		...
		if (isset($post['submit'])) {
			...
			// 		$file_content = file_get_contents($_FILES["dokumen_sip"]["tmp_name"]);
			...
			// 		$file_content = file_get_contents($_FILES["dokumen_str"]["tmp_name"]);
			// 		if (strpos($file_content, '<?php') !== false) {
			// 			// die("File contains PHP code.");
			// 			$this->session->set_flashdata('kode_name', 'alert alert-danger alert-dismissible');
			// 			$this->session->set_flashdata('icon_name', 'warning');
			...
			// 		$file_content = file_get_contents($_FILES["dokumen_kewenangan"]["tmp_name"]);
			// 		if (strpos($file_content, '<?php') !== false) {
			// 			// die("File contains PHP code.");
			// 			$this->session->set_flashdata('kode_name', 'alert alert-danger alert-dismissible');
			// 			$this->session->set_flashdata('icon_name', 'warning');
			...
			// 		$file_content = file_get_contents($_FILES["dokumen_komitmen"]["tmp_name"]);
			// 		if (strpos($file_content, '<?php') !== false) {
			// 			// die("File contains PHP code.");
			// 			$this->session->set_flashdata('kode_name', 'alert alert-danger alert-dismissible');
			// 			$this->session->set_flashdata('icon_name', 'warning');
			...
			$tgl_berakhir = $post['tgl_berakhir_sip'];

			if ($tgl_berakhir != NULL) {
				$tgl_berakhir_sip = date('Y-m-d', strtotime($post['tgl_berakhir_sip']));
			} else {
				$tgl_berakhir_sip = NULL;
			}
			$datas = array(
				...
				'id_faskes' => $post['id_faskes'],
				'no_sip' => $post['no_sip'],
				'dokumen_sip' => $post['dokumen_sip'],
				'sip_ke_berapa' => $post['sip_ke_berapa'],
				'tgl_berakhir_sip' => $tgl_berakhir_sip,
				'no_str' => $post['no_str'],
				'dokumen_str' => $post['dokumen_str'],
				...
				'dokumen_registrasi' => $post['dokumen_registrasi'],
				...
				'dokumen_komitmen' => $post['dokumen_komitmen'],
				//baru
				'berjejaring_puskesmas' => $post['berjejaring_puskesmas'],
				'dokumen_kewenangan' => $post['dokumen_kewenangan'],
			...
			if (!empty($post['id'])) {
				...
				$query = $dbmutu->query("update user set nama_lengkap='" . $post['nama_pm'] . "' where username='" . $post['kode'] . "'");
				$query2 = $dbmutu->query("update m_satker set nama='" . $post['nama_pm'] . "' where kode_satker='" . $post['kode'] . "'");
			} else {
	...
	{ {
			...
			$link = 'https://registrasifasyankes.kemkes.go.id/assets/uploads/QRAPI/' . $fc . '.png';
			...
			//var_dump($result);
			//echo $result;
			file_put_contents("assets/uploads/QRAPI/" . $fc . ".png", $result);
	...
	function rekap_data_pm($id = NULL)
	{
		...
		$this->template->utama('admin/rekap_data_pm', $data);
	}
	...
	public function index_data_rme()
	{
		...
		$data['satu_sehat'] = $this->reviewmodel->get_satu_sehat($this->session->userdata('user_id'));
		$data['data3'] = $this->reviewmodel->get_satu_sehat_pic($this->session->userdata('user_id'));
		$data['odelia'] = $this->reviewmodel->get_api_odelia($this->session->userdata('user_id'));
		...
		if (isset($post['submit'])) {
			...
			if ($jenis_vendor_lama != $jenis_vendor_baru || $vendor_lama != $vendor_baru) {
				...
				$datavendor = $this->registrasiusermodel->getdatavendor($vendor_baru);
				$vendor_baru_dto = $datavendor[0]['dtoId'];
				$nama_vendor_dto = $datavendor[0]['nameFacility'];

				$postData = json_encode(array(
					"email" => $data['data3'][0]['email_integrasi'],
					"facilityCode" => $data['satu_sehat'][0]['kode_baru_faskes'],
					"orgId" => $data['satu_sehat'][0]['organization_id'],
					"vendorId" => (int)$vendor_baru_dto
				));
				...
				if (json_decode(http_response_code()) === 200) {
					...
					$datasatusehat = array(
						...
						'sim_pengembang_id' => $vendor_baru_dto,
						'sim_pengembang_nama' => $nama_vendor_dto
					);
					...
					// redirect(base_url('pm/index_data_rme'));
					if ($updateData['message'] == 'user data is invalid') {
						$this->session->set_flashdata('kode_name', 'danger');
						$this->session->set_flashdata('icon_name', 'times');
						$this->session->set_flashdata('message_name', 'Gagal Update Data RME, Silakan Mengubah Email Kontak SatuSehat dengan Email Institusi');
					} else {
						$this->session->set_flashdata('kode_name', 'danger');
						$this->session->set_flashdata('icon_name', 'check');
						$this->session->set_flashdata('message_name', 'Gagal Simpan, karena baru saja melakukan pergantian RME dalam 24 jam terakhir.');
			...
			elseif($post['rme'] == 1 && $post['sim_pengembang_id'] == 1012){


				$where3 = array('kode_faskes' => $data['data2'][0]['kode_faskes']);
				$cek_kode = $this->registrasiusermodel->select_data('satu_sehat', $where3)->result_array();
				
				// $this->session->set_flashdata('kode_name', 'alert alert-primary alert-dismissible');
				// 		$this->session->set_flashdata('icon_name', 'primary');
				// 		$this->session->set_flashdata('message_name', $cek_kode);
				
				// print_r($cek_kode);

				$timestamp = time();
				$api_key = "kyzsnzqinca7ar25b4edjmfgvmu3zfe3";
				$secret = "y5li8uv1jm";
				$email = $data['user'][0]['email'];
				$kode_faskes = $data['data3'][0]['kode_faskes'];
				$nama_fasyankes = $data['data'][0]['nama_pm'];
				$email_integrasi = $data['data3'][0]['email_integrasi'];
				$pic_satusehat = $data['data3'][0]['nama_pic'];
				$no_telp_satusehat = $data['data3'][0]['telp_pic'];
				$lat = $data['data'][0]['latitude'];
				$long = $data['data'][0]['longitude'];
				$full = $api_key.$secret.$timestamp;
				$signature = md5($full);
				...
				$dataapis = array(
					...
					'email_satusehat' => $email_integrasi,
					'pic_satusehat' => $pic_satusehat,
					'no_telp_satusehat' => $no_telp_satusehat,
					'lat' => $lat,
					'long' => $long,
					'status_kirim' => 1,
					'jumlah_kirim' => 1
				...
				if (!empty($cek_kode)) {
					if (!empty($post['id'])) {
						...
						$where2 = $data['data3'][0]['kode_faskes'];
						
						$curl = curl_init();

						curl_setopt_array($curl, array(
						CURLOPT_URL => 'https://public.bidan-delima.id',
						CURLOPT_RETURNTRANSFER => true,
						CURLOPT_ENCODING => '',
						CURLOPT_MAXREDIRS => 10,
						CURLOPT_TIMEOUT => 0,
						...
						CURLOPT_HTTPHEADER => array(
							...
							'api-key: kyzsnzqinca7ar25b4edjmfgvmu3zfe3',
						...
						// $jsonDecodeResponse = json_decode($response, true);
						$jsonDecodeResponse = json_decode(http_response_code());
						// print_r($jsonDecodeResponse);
						// print_r(json_decode(http_response_code());
						
						if (json_decode(http_response_code()) === 200) {
							
							$where7 = array('kode_faskes' => $data['data2'][0]['kode_faskes']);
							$cek_api = $this->registrasiusermodel->select_data('api_odelia', $where7)->result_array();
							...
							if (!empty($cek_api)) {
								$this->registrasiusermodel->edit_data('api_odelia',$where7, $dataapis);
								$this->session->set_flashdata('kode_name', 'alert alert-danger alert-dismissible');
								$this->session->set_flashdata('icon_name', 'primary');
								$this->session->set_flashdata('message_name', $response);
								redirect('pm/index_data_rme');
							}else{
								$this->registrasiusermodel->input_data('api_odelia', $dataapis);
								$this->session->set_flashdata('kode_name', 'alert alert-danger alert-dismissible');
								$this->session->set_flashdata('icon_name', 'primary');
								$this->session->set_flashdata('message_name', $response);
								redirect('pm/index_data_rme');
						...
						$this->registrasiusermodel->input_data('timeline', $datas_log);
						$this->registrasiusermodel->input_data('api_odelia', $dataapi);

						$this->session->set_flashdata('kode_name', 'alert alert-danger alert-dismissible');
						$this->session->set_flashdata('icon_name', 'primary');
						$this->session->set_flashdata('message_name', 'Berhasil Update Data');
						redirect('pm/index_data_rme');
					...
					redirect('pm/index_data_rme');
						...
						// $this->session->set_flashdata('kode_name', 'warning');
						// $this->session->set_flashdata('icon_name', 'check');
						// // $this->session->set_flashdata('message_name', $timestamp);
						// $this->session->set_flashdata('message_name', $response);
						// redirect(base_url('pm/index_data_rme'));
	...
	public function api_odelia()
	{
		...
		$post = $this->input->post();
			...
			$datas = array(
				...
				$full = $api_key.$secret.$timestamp;
				$signature = md5($full);
				$kirim = $data['odelia'][0]['jumlah_kirim'];
					...
					'jumlah_kirim' => $kirim + 1
						...
						if (json_decode(http_response_code()) === 200) {
							...
							$cek_api = $this->registrasiusermodel->select_data('api_odelia', $where7)->result_array();
								...
								$this->registrasiusermodel->edit_data('api_odelia',$where7, $dataapis);
								$this->session->set_flashdata('kode_name', 'alert alert-warning alert-dismissible');
								$this->session->set_flashdata('icon_name', 'warning');
								$this->session->set_flashdata('message_name', $response);
								redirect('pm/index_data_rme');
	...
	public function pcare()
	{
		...
		if (isset($post['submit'])) {
			...
			$datas = array(
				...
				'id_faskes' => $post['id_faskes'],
				'dokumen_integrasi_pcare' => $post['dokumen_integrasi_pcare']
			);
	...
	public function index_daftar_usulan()
	{
		...
		$datacek=str_replace('"', '', $cek);


// $curl = curl_init();

// curl_setopt_array($curl, array(
//   CURLOPT_URL => 'https://mutufasyankes.kemkes.go.id/api/status_verif_katim/'.$datacek,
//   CURLOPT_RETURNTRANSFER => true,
//   CURLOPT_ENCODING => '',
//   CURLOPT_MAXREDIRS => 10,
//   CURLOPT_TIMEOUT => 0,
...
//     'X-Id: mutukemenkes',
//     'X-Pass: rsonline!@#$',
//     'X-Timestamp: 1727148583',
//     'Cookie: TS01151b7a=0172bf5c62ab53ca08524801534637a6e22fc6177beb6bba54bda16aff19c4200fc34e92f5c29a4f2e9ac351e4a72cfd12797936c48dc187d783d91cea820f4fb7b37eb665; ci_session=a%3A5%3A%7Bs%3A10%3A%22session_id%22%3Bs%3A32%3A%22119209519a20c55b2b640282077216a6%22%3Bs%3A10%3A%22ip_address%22%3Bs%3A14%3A%22172.31.141.118%22%3Bs%3A10%3A%22user_agent%22%3Bs%3A21%3A%22PostmanRuntime%2F7.41.2%22%3Bs%3A13%3A%22last_activity%22%3Bi%3A1727145119%3Bs%3A9%3A%22user_data%22%3Bs%3A0%3A%22%22%3B%7Da43119ab426ffe68e960f15b2eeb67be'
//   ),
// ));
...
// $jsonDecodeToken = json_decode($response, true);
// $clientKeya = $jsonDecodeToken[0]['keterangan_katim'];
// $data['api'] = $clientKeya;
// print_r($data['usulan']);
// echo $clientKeya = json_encode($jsonDecodeToken[0]['keterangan_katim']);
$this->template->utama('datapm/index_daftar_usulan', $data);
	...
	public function inputan_usulan_akreditasi()
	{
		...
		$post = $this->input->post();
		// var_dump($post);
		if($post['tanggal'] == NULL){
			$this->session->set_flashdata('error', 'Silahkan isikan dulu tanggal usulannya!');
		}else{
		if (isset($post['cek'])) {
			...
			$data['data_review'] = $this->akreditasipmmodel->get_review($this->session->userdata('user_id'));
			$data['data_review_star'] = $this->akreditasipmmodel->get_review_star($this->session->userdata('user_id'));
	...
	public function pic_faskes()
	{
		...
		$data['data4'] = $this->reviewmodel->get_satu_sehat_pic($this->session->userdata('user_id'));
		$data['satu_sehat'] = $this->reviewmodel->get_satu_sehat($this->session->userdata('user_id'));
		$data['rme'] = $this->registrasiusermodel->getdatarmepm($this->session->userdata('user_id'));
